...
- [Anti-Hallucination Framework — 从哲学到执行的六层防御体系](docs/ANTI-HALLUCINATION_FRAMEWORK.md)

  底层逻辑（概率生成→幻觉不可消除）→ 认知科学（认知卸载→信任跳级）→ 核心悖论（知识悖论→验证存在）→ 元规则（AI自查自证）→ 执行规则（五类强检）→ 制度性补偿（流程替人警惕）
